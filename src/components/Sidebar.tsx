...
interface SidebarProps {
    onViewChange: (view: 'dashboard' | 'inventory' | 'sales' | 'analytics') => void;
    currentView: string;
}
                ...
                <ul style={{ listStyle: 'none' }}>
                    {navItems.map((item) => (
                        <li key={item.id} style={{ marginBottom: '0.5rem' }}>
                            <button
                                onClick={() => (item.id === 'dashboard' || item.id === 'inventory' || item.id === 'sales' || item.id === 'analytics') && onViewChange(item.id as any)}
                                ...
                                style={{
                                    ...
                                    textAlign: 'left',
                                    cursor: (item.id === 'dashboard' || item.id === 'inventory' || item.id === 'sales' || item.id === 'analytics') ? 'pointer' : 'not-allowed',
                                    opacity: (item.id === 'dashboard' || item.id === 'inventory' || item.id === 'sales' || item.id === 'analytics') ? 1 : 0.5
                                }}
                            >
                                <item.icon size={20} />
